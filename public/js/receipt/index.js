...
$(function () {
    dateTimePickerMonthYear($("#time-receipt"));
    loadDataType();
    loadData();
    $('#select-type-receipt').on('change', function () {
        loadData();
    })
})
...
async function loadData() {
    let method = "get",
        url = "receipt.data",
        params = {
            from: moment($('#time-receipt').val(), 'MM/YYYY').format('MM/01/YYYY'),
            to: moment($('#time-receipt').val(), 'MM/YYYY').endOf('month').format('MM/DD/YYYY'),
            type: $('#select-type-receipt').val()
        },
        data = null;
...
async function loadDataType() {
    let method = "get",
        url = "receipt.data-type",
        params = null,
        data = null;
    let res = await axiosTemplate(method, url, params, data, [$('body')]);
    $('#select-type-receipt').append(res.data[0]).trigger("chosen:updated");
    $('#type-create-receipt').html(res.data[0]).trigger("chosen:updated");
    $('#type-update-receipt').html(res.data[0]).trigger("chosen:updated");
}

async function dataTable(data) {
    let id = $("#table-receipt"),
        fixed_left = 0,
        fixed_right = 0,
        columns = [
